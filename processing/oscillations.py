...
def get_power_by_band(session, best_bla_channels,oscillations_bands):
    ...
    for side,chan in best_bla_channels.items():
        ...
        f,t_spec,Sxx = spectrogram(lfp[side],1250,nperseg=5000,noverlap=2500)
...
def process_session(base_folder: Union[Path, str] = upath['base_folder'],
    ...
    states = load.sleep_scoring(session,discard=['WAKE','DROWSY'],drop_short_intervals=min_durations)
    ...
    if save:
        save_data(session,power,best_bla_channels,nbins,min_durations,oscillations_bands)
    ...
    for side,c_averaged in session['power']['averaged'].items():
        ...
        for band in bands:
            for state in prev_concat_session[band].keys():
                prev_concat_session[band][state].extend(c_averaged[band][c_averaged['state'] == state].values)
