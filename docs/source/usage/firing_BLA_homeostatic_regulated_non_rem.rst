...
1. Execute :py:mod:`~processing/fr_states.py` :
...
This will load, session by session, the data set and compute  TO COMPLETE

2. Execute :py:mod:`~plots.plot_delta_fr.py`
...
p = 4.1 × 10−5 ). Right: Same as Left but data points are group in 10 same sized group. Error bar shows SEM. (B) Left:
...
and REM sleep. Correlation with time spent in NREM or REM ( r = −0.15, p = 4.05 × 10^{−13} ; r = −0.06, p = 0.17 ). Correlation
with wake firing rates and delta during NREM or REM ( r = −0.17, p = 2.44 × 10^{−7} ; r = −0.04, p = 0.21 ). Neurons that fire the
